...
func HuaweiResponse(cxt *Context) {
	...
	inputAvgRateVendorAttr := VendorAttr{
		VendorType:   2,
		VendorLength: 6,
		VendorValue:  common.GetIntegerBytes(uint32(upStreamLimit)),
	}
	...
	outputAvgRateVendorAttr := VendorAttr{
		VendorType:   5,
		VendorLength: 6,
		VendorValue:  common.GetIntegerBytes(uint32(downStreamLimit)),
	}
	...
	if product.DomainName != "" {
		domainNameVendorAttr := VendorAttr{
			VendorType:   138,
			VendorLength: 66,
			VendorValue:  common.FillBytesByString(64, product.DomainName),
		}
...
func AuthSpecAndCommonAttrSetter(cxt *Context) {
	...
	sessionTimeoutAttr := RadiusAttr{
		...
		AttrType: 27,
		// 默认会话时长一星期
		AttrValue: common.GetIntegerBytes(uint32(cxt.User.SessionTimeout)),
	}
